<script lang="ts">
  ...
  $: subtitle = [termName, courses.length ? `${courses.length} 个课程时段` : '还没有导入课表'].filter(Boolean).join(' · ');
</script>
...
<section class="page page-week">
  ...
  <div class="week-board content-surface" style={`--section-count:${sectionCount}`}>
    <div class="week-header">
      ...
      {#each days as day, i}<div class:today={i === todayIndex}><span>{day.label}</span><b>{day.date}</b></div>{/each}
    </div>
