...
class LumaBridge(
    ...
    fun request(id: String, command: String, payload: String?) {
        ...
        executor.execute {
            runCatching { route(command, payload.orEmpty()) }
                ...
                .onFailure { resolve(id, false, JSONObject().put("message", it.message ?: "Native command failed").toString()) }
        }
    }
        ...
        return when (command) {
            "get_bootstrap" -> {
                ...
                JSONObject()
                    ...
                    .put("glassSettings", glass?.let { runCatching { JSONObject(it) }.getOrNull() } ?: JSONObject.NULL)
                    .put("dbReady", true)
                    .put("nativeCore", true)
            ...
            "save_schedule_course" -> JSONObject().put("value", database.saveScheduleCourse(args.getJSONObject("course"))).toString()
            "delete_schedule_course" -> {
                database.deleteScheduleCourse(args.getString("id")); "null"
            }
            "save_glass_settings" -> {
                database.setSettingRaw("appearance.glass", args.getJSONObject("settings").toString()); "null"
            }
            ...
            "get_course_reminder_settings" -> {
                database.getSettingRaw("reminders.course.default") ?: JSONObject().put("enabled", false).put("offsetMinutes", 15).toString()
            }
    ...
    private fun updateWidget(snapshot: JSONObject): Boolean {
        ...
        prefs.edit()
            ...
            .putString("next_course_meta", snapshot.optString("courseMeta", "打开 LumaSchedule 查看课表"))
    ...
    private fun sendTestNotification(): Boolean {
        ...
        ReminderReceiver().onReceive(
            ...
            Intent(activity, ReminderReceiver::class.java).apply {
                ...
                putExtra(ReminderReceiver.EXTRA_BODY, "测试通知发送成功。课程提醒可以正常显示。")
            }
        )
        return true
    }
    ...
    private fun scheduleReminder(reminder: JSONObject): Boolean {
        ...
        activity.getSharedPreferences(BootReceiver.PREFS_NAME, Context.MODE_PRIVATE)
            .edit().putString(BootReceiver.key(id), stored.toString()).apply()
        return true
    }
